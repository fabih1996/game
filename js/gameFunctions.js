...
// Only list truly known NPCs or the player—skip any generic “ghost” entries
  characters
   ...
   wrapper.className = "char-wrapper";          // nuovo wrapper standard
     ...
     // -- prima calcoliamo matchedName ------------------------------
   const matchedName = Object.keys(characterColors)
     .find(known => name.toLowerCase().includes(known.toLowerCase()));
   
   // colore dell’anello (usa la tabella o fallback)
   wrapper.style.setProperty(
     "--ring-color",
     characterColors[matchedName || name] || characterColors.default
   );
